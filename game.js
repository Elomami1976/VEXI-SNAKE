...
/* ---------- Controls ---------- */
function startGame() {
  if (running) return; // guard: ignore double-starts (click + touch)
...
// tap anywhere on the board or overlay to start / restart
canvas.addEventListener("click", () => { if (!running) startGame(); });
overlay.addEventListener("click", (e) => {
  if (e.target !== btnStart && !running) startGame();
});
...
reset();
draw();
// force the correct start screen, even over stale/cached markup
if (!running) showOverlay("SNAKE", "Eat the red food.<br>Don't hit walls or yourself.", "PLAY");
