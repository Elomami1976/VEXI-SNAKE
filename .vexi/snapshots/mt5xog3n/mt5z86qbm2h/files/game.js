...
let snake, dir, nextDirs, food, score, best, speed, alive, running;
...
function reset() {
  ...
  nextDirs = [];
  score = 0;
  speed = 130;          // ms per step (lower = faster)
  alive = true;
  placeFood();
  updateScore();
}
...
function step() {
  ...
  if (head.x === food.x && head.y === food.y) {
    ...
    speed = Math.max(60, 130 - score * 1.2);
    placeFood();
  } else {
    snake.pop();
  }
...
document.addEventListener("keydown", (e) => {
  ...
  if (dirs[k]) {
    ...
    else queueDir(dirs[k]);
  }
  if (k === "p" && running && alive) paused = !paused;
});
...
function queueDir(d) {
  ...
  if (nextDirs.length < 2) nextDirs.push(d);
}
...
canvas.addEventListener("touchend", (e) => {
  ...
  if (!running) startGame();
  else queueDir(Math.abs(dx) > Math.abs(dy)
    ? { x: Math.sign(dx), y: 0 }
    : { x: 0, y: Math.sign(dy) });
  touchX = touchY = null;
});
